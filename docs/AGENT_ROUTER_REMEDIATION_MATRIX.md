...
| A6 | Update root README, Elixir README, and workflow documentation when behavior/config changes. | Task 6 docs review covers retry/review/CI policy in root `README.md`, `elixir/README.md`, and `elixir/WORKFLOW.md` | Retry limits, capacity accounting, disabled CI retry, and reload/restart boundary are documented alongside routed lifecycle behavior | `mise exec -- make all` passed; `git diff --check` clean | Docs cannot substitute for live operational proof |
...
| SYM-01 | Planner reads and produces a bounded plan without source writes. | Existing lifecycle proof plus `role_prompt_test.exs` role-permission assertions | Shipped planner prompt declares planning responsibility, read-only runtime permission, bounded planning output, and no source/approval/merge authority. Full gate: 380 tests, 0 failures, 6 skipped. |
| SYM-02 | Reviewer is read-only and reports exact PASS/FAIL/BLOCKED evidence. | Existing lifecycle proof plus `role_prompt_test.exs` reviewer-permission assertions | Shipped reviewer prompt declares read-only permission and exact PASS/FAIL/BLOCKED output with no source, approval, or merge authority. Full gate: 380 tests, 0 failures, 6 skipped. |
...
| SYM-04 | Changes Requested correction is bounded, reviewed, and retry-limited. | `core_test.exs` simulates four reviewer→correction route boundaries with real orchestrator state and asserts the fourth is blocked | `AttemptPolicy` caps review cycles at three; route changes remain separate; exhausted work is retained as human-attention blocked state | Affected suite and full gate: `389 tests, 0 failures, 6 skipped`; 100% total coverage | Component proof only; live provider transitions unavailable |
...
| SYM-13 | Prompt reload and shipped workflow are coherent. | `role_prompt_test.exs` proves runtime reload, packaged fallback, active-attempt stability, role coherence, and routed `WORKFLOW.md` parsing | Prompt files are runtime-first with embedded fallback; shipped workflow opts into routed profiles and removes the conflicting legacy lifecycle body. Full gate: 380 tests, 0 failures, 6 skipped. |
| SYM-14 | Retry/review/CI limits are explicit and enforced. | `attempt_policy_test.exs` and `core_test.exs` assert three-retry/three-cycle thresholds, capacity separation, terminal/reload behavior, and CI disabled | `AgentRuntime.AttemptPolicy` exposes the three-retry/three-cycle limits and `ci_retry_policy/0 == :disabled`; orchestrator emits human-attention blocked records at threshold | `mise exec -- make all`: `389 tests, 0 failures, 6 skipped`; 100% total coverage; Credo clean; Dialyzer 0 errors | Live proof blocked; no disposable provider resources authorized |
...
| Task 5 | Affected prompt/runtime/config suite: `144 tests, 0 failures`; full run: `380 tests, 0 failures, 6 skipped`; 100% total coverage | `mise exec -- make all`: passed; Credo clean; Dialyzer 0 errors | Clean |
| Task 6 | Affected policy/orchestrator/core suite: `69 tests, 0 failures`; full run: `389 tests, 0 failures, 6 skipped`; 100% total coverage | `mise exec -- make all`: passed; Credo clean; Dialyzer 0 errors | Clean |
| 7–10 | Pending | Pending | Pending |
